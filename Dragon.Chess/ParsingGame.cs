...
namespace Dragon.Chess {
    public class ParsingGame {
        ...
        public ParsingGame(string body) {
            this.Position = new Piece[128];
            this.ColorToMove = 1;
            this.StartPly = 1;
            this.StartFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
            this.CurrentPosition = Dragon.Chess.Position.StartPosition;
            this.Body = body;
        }
        ...
        /// <summary>
        /// Used to indicate that the game is a placeholder with just a result
        /// </summary>
        ...
        private struct ChessRegex {
            ...
            public static Regex MoveBodyExtra = new Regex("(.*?[a-h][1-8](?:=[QBRN])?[+#]?)(.*)", RegexOptions.Compiled); // only check and checkmate are allowed as literals, everything else must be a nag or stripped
                ...
                for (var i = 0; i < 8; i++) {
                    if (rows[col].Length > ixChar) {
                        ...
                        if (char.IsLetter(charNext)) {
                            ...
                            this.Position[offset + i] = (Piece)(color * (int)piece);
        ...
        private void ParsePgn(string pgn) {
            ...
            this.Moves[0].Transitions = new List<Transition>();
            ...
            if (firstPly > 0) {
                this.ExtractMoves(0, firstPly, pgn, false);
            }
        ...
        private static string PatternForPly(int ply) {
            return ((ply + 1) / 2).ToString() + (ply % 2 == 1 ? "\\." : "\\.\\.\\.");
        }

        private static object _lock = new object();
        private static Dictionary<(int, bool), Regex> _regexes = new Dictionary<(int, bool), Regex>();

        private static Regex GetVariationRegex(int ply) {
            if (!_regexes.ContainsKey((ply, false))) {
                lock (_lock) {
                    // get the number (ply 1 = 1., ply 2 = 1..., etc.)
                    var numberPattern = PatternForPly(ply);

                    // process all the variations for this particular ply
                    var variationPattern = "%%(\\d+)%\\s*" + numberPattern + "\\s*(\\w\\S+)" + ChessRegex.Suffix + "(?:\\s+(.*?))?%\\1%%";

                    _regexes[(ply, false)] = new Regex(variationPattern, RegexOptions.Compiled);
                }
            }

            return _regexes[(ply, false)];
        }

        private static Regex GetMainRegex(int ply) {
            if (!_regexes.ContainsKey((ply, true))) {
                lock (_lock) {
                    var numberPattern = PatternForPly(ply);
                    var mainPattern = "(?:^|\\s+)" + numberPattern + "\\s*(\\w\\S+)" + ChessRegex.Suffix + "(?:\\s+(.*)|($))";
                    _regexes[(ply, true)] = new Regex(mainPattern, RegexOptions.Compiled);
                }
            }

            return _regexes[(ply, true)];
                
        }

        private void ExtractMoves(int parentId, int ply, string pgn, bool inVariation) {
            //var numberPattern = PatternForPly(ply);
            string numberPattern = ((ply + 1) / 2).ToString() + (ply % 2 == 1 ? "." : "...");
            var variationRegex = GetVariationRegex(ply);
            ...
            foreach (Match m in variationRegex.Matches(pgn)) {
                ...
                if (comment.StartsWith("rare move")) {
                    var a = 0;
                }

                // store in the moves dictionary
                this.ParseCurrentMoveId++;
                ParsingMove move = new ParsingMove(this.ParseCurrentMoveId, ply, moveBody, parentId, numberPattern, comment, nag);
                ...
                if (!string.IsNullOrEmpty(line)) {
                    ...
                    if (line.Length > 0 && char.IsLetter(line[0])) {
                        ...
                        line = PatternForPly(ply + 1).Replace("\\", "") + " " + line;
                    }
                    // move a level deeper within the variation
                    this.ExtractMoves(this.ParseCurrentMoveId, ply + 1, line, true);
                } else {
                    // add a finishing move
                    int resultParent = this.ParseCurrentMoveId;
                    this.ParseCurrentMoveId++;
                    ParsingMove resultMove = new ParsingMove(this.ParseCurrentMoveId, ply + 1, "", resultParent, "");
                    resultMove.IsVariation = true;
                    this.Moves.Add(this.ParseCurrentMoveId, resultMove);
                    this.Moves[resultParent].Children.Add(resultMove);
                }
            }
            ...
            var mainRegex = GetMainRegex(ply);
            ...
            if (main.Success) {
                ...
                if (!string.IsNullOrEmpty(line)) {
                    ...
                    // move a level deeper
                    this.ExtractMoves(this.ParseCurrentMoveId, ply + 1, line, inVariation);
                } else {
                    // add a finishing move
                    int resultParent = this.ParseCurrentMoveId;
                    this.ParseCurrentMoveId++;
                    ParsingMove resultMove = new ParsingMove(this.ParseCurrentMoveId, ply + 1, "", resultParent, "");
                    resultMove.IsVariation = inVariation;
                    this.Moves.Add(this.ParseCurrentMoveId, resultMove);
                    this.Moves[resultParent].Children.Add(resultMove);
                }
            } else {
        ...
        private void ExecuteMoveForward(ParsingMove move) {
        ...
        private void ExecuteMoveBackward() {
        ...
        private void ExecuteForwardTransitions(ParsingMove move) {
            ...
            this.EnPassant = null;
            foreach (Transition t in move.Transitions) {
                switch (t.Type) {
                    case TransitionType.Move:
                        Position[t.SecondSquare] = Position[t.FirstSquare];
                        if (Position[t.SecondSquare] == Piece.WhiteKing) {
                            this.WhiteKingPosition = t.SecondSquare;
                        }
                        if (Position[t.SecondSquare] == Piece.BlackKing) {
        ...
        private void ExecuteBackwardTransitions(ParsingMove move) {
            ...
            this.EnPassant = null;
            foreach (Transition tran in Moves[move.ParentMoveId].Transitions.Where(t => t.Type == TransitionType.EnPassant)) {
            ...
            for (int i = move.Transitions.Count - 1; i >= 0; i--) {
                ...
                switch (t.Type) {
                    case TransitionType.Move:
                        ...
                        if (Position[t.FirstSquare] == Piece.WhiteKing) {
                            this.WhiteKingPosition = t.FirstSquare;
                        }
                        if (Position[t.FirstSquare] == Piece.BlackKing) {
                    ...
                    case TransitionType.PawnPromo:
                        ...
                        Position[t.FirstSquare] = (Piece)((int)ColorlessPiece.Pawn * this.ColorToMove * -1);
                        break;
        ...
        private void GenerateAndExecuteTransitions(ParsingMove move) {
            var parentPosition = this.CurrentPosition;
            move.Position = this.CurrentPosition;

            if (move.MoveBody == "") {
                // special case for the result move
                return;
            }

            var legalMoves = MoveGenerator.LegalMoves(move.Position, move.Position.WhiteToMove, filter: move.MoveBody);
            var m = GetLegalMove(move, legalMoves);
            move.LegalMove = m;

            if (m.ResultingPosition != null) {
                this.CurrentPosition = m.ResultingPosition;
            } else {
                var next = MoveGenerator.CreateNextPosition(move.Position, m);
                this.CurrentPosition = next;
            }

            foreach (var child in move.Children) {
                GenerateAndExecuteTransitions(child);
            }

            this.CurrentPosition = parentPosition;
        }

        private Move GetLegalMove(ParsingMove move, IEnumerable<Move> legalMoves) {
            string clean = move.MoveBody.TrimEnd('+', '#', ' ', '!', '?');

            var matches = legalMoves.Where(m => m.San == clean);
            if (matches.Count() == 1) {
            ...
            foreach (var m in legalMoves) {
                if (m.RankSan == null || m.FileSan == null) {
                    continue;
                }
                if (m.RankSan == clean || m.FileSan == clean) {
                    return m;
                }
            }

            throw new PgnException("Could not match " + move.MoveBody);
        }
        ...
        //    move.Position.Squares = new Piece[128];
        //    this.Position.CopyTo(move.Position.Squares, 0);
        //    move.Position.ColorToMove = this.ColorToMove;
        //    move.Position.WhiteCastling = (Castling)this.WhiteCastling;
        //    move.Position.BlackCastling = (Castling)this.BlackCastling;
        //    move.Position.EnPassant = this.EnPassant;
        ...
        /*
        private List<Transition> GenerateTransitions(ParsingMove move) {
            ...
            if (ChessRegex.CastleQueenside.IsMatch(move.MoveBody)) {
                ...
                transitions.Add(new Transition(TransitionType.Move, kstart, kstart - 2));
                transitions.Add(new Transition(TransitionType.Move, rstart, rstart + 3));
                transitions.Add(new Transition(TransitionType.Castling, (byte)oldCastling, (byte)Castling.None));
                move.HalfMoveCount = 0;

                move.UciStart = kstart;
                move.UciEnd = rstart;
                move.UciPiece = (int)ColorlessPiece.King;
            ...
            if (ChessRegex.CastleKingside.IsMatch(move.MoveBody)) {
                ...
                transitions.Add(new Transition(TransitionType.Move, kstart, kstart + 2));
                transitions.Add(new Transition(TransitionType.Move, rstart, rstart - 2));
                transitions.Add(new Transition(TransitionType.Castling, (byte)oldCastling, (byte)Castling.None));
                move.HalfMoveCount = 0;
        ...
        private List<Transition> GeneratePawnMoveTransitions(ParsingMove move) {
            var transitions = new List<Transition>();
            ...
            if (ChessRegex.PawnMove.IsMatch(move.MoveBody)) {
                ...
                if (this.Position[minusOneStartSquare] == (Piece)((sbyte)ColorlessPiece.Pawn * ColorToMove)) {
                    // yes, we moved forward one square!
                    ixStartSquare = minusOneStartSquare;

                    transitions.Add(new Transition(TransitionType.Move, ixStartSquare, (trank * 16 + tfile)));

                    move.UciStart = ixStartSquare;
                    move.UciEnd = (trank * 16 + tfile);
                    move.UciPiece = (int)ColorlessPiece.Pawn;
                    ...
                    if (this.ColorToMove == 1) {
                        // white is moving
                        if (trank == 3 && Position[16 + tfile] == Piece.WhitePawn && Position[32 + tfile] == (sbyte)Piece.Empty) {
                            int toSquare = 48 + tfile;
                            transitions.Add(new Transition(TransitionType.Move, (16 + tfile), toSquare));

                            // if we have a black pawn adjacent, add the en passant transition
                            if (Position[toSquare - 1] == Piece.BlackPawn || Position[toSquare + 1] == Piece.BlackPawn) {
                                transitions.Add(Transition.EnPassant(tfile));
                            }

                            move.UciStart = (16 + tfile);
                            move.UciEnd = toSquare;
                            move.UciPiece = (int)ColorlessPiece.Pawn;
                        ...
                        // black is moving
                        if (trank == 4 && Position[96 + tfile] == Piece.BlackPawn && Position[80 + tfile] == (sbyte)Piece.Empty) {
                            int toSquare = 64 + tfile;
                            transitions.Add(new Transition(TransitionType.Move, (96 + tfile), toSquare));

                            // if we have a black pawn adjacent, add the en passant transition
                            if (Position[toSquare - 1] == Piece.WhitePawn || Position[toSquare + 1] == Piece.WhitePawn) {
                                transitions.Add(Transition.EnPassant(tfile));
                            }

                            move.UciStart = (96 + tfile);
                            move.UciEnd = toSquare;
                            move.UciPiece = (int)ColorlessPiece.Pawn;
                ...
                if (this.EnPassant == tfile) {
                    ...
                    if (remSquare != 0) {
                        ...
                        transitions.Add(Transition.Remove(remSquare, remPiece));
                        transitions.Add(new Transition(TransitionType.Move, minusOneRank * 16 + ffile, trank * 16 + tfile));
                        move.HalfMoveCount = 0;

                        move.UciStart = minusOneRank * 16 + ffile;
                        move.UciEnd = trank * 16 + tfile;
                        move.UciPiece = (int)ColorlessPiece.Pawn;
                ...
                transitions.Add(Transition.Remove(trank * 16 + tfile, Position[trank * 16 + tfile]));
                transitions.Add(new Transition(TransitionType.Move, ixStartSquare, trank * 16 + tfile));
            ...
            if (ChessRegex.PawnPromoSuffix.IsMatch(move.MoveBody)) {
                ...
                Piece promo = (Piece)((sbyte)promoPiece * ColorToMove);
                transitions.Add(Transition.PawnPromotion(trank * 16 + tfile, promo));
                //transitions.Add(new Transition(TransitionType.PawnPromo, trank * 16 + tfile, (sbyte)promoPiece * ColorToMove));

                move.UciPiece = Math.Abs((int)promo);
        ...
        private List<Transition> GeneratePieceMoveTransitions(ParsingMove move) {
            var transitions = new List<Transition>();
            ...
            if (Position[ixEndSquare] != 0) {
                if (this.ColorToMove > 0 && Position[ixEndSquare] > 0) {
                    throw new PgnException("Cannot move to occupied square");
                }
                if (this.ColorToMove < 0 && Position[ixEndSquare] < 0) {
                    throw new PgnException("Cannot move to occupied square");
                }
            ...
            if (possibleOrigins.Count == 1) {
                ...
                transitions.Add(new Transition(TransitionType.Move, ixStartSquare, ixEndSquare));
            } else {
            ...
            if (this.ColorToMove == 1) {
                ...
                // a rook
                if (ixStartSquare == 0 && (((byte)WhiteCastling & 1) > 0)) {
                ...
                // h rook
                if (ixStartSquare == 7 && (((byte)WhiteCastling & 2) > 0)) {
                    transitions.Add(new Transition(
                        TransitionType.Castling,
                        (Castling)WhiteCastling,
                ...
                if (ixStartSquare == 4 && WhiteCastling != (int)Castling.None) {
                    transitions.Add(new Transition(TransitionType.Castling, (Castling)WhiteCastling, Castling.None));
                }
                ...
                // a rook
                if (ixStartSquare == 112 && (((byte)BlackCastling & 1) > 0)) {
                    transitions.Add(new Transition(
                        TransitionType.Castling,
                ...
                // h rook
                if (ixStartSquare == 119 && (((byte)BlackCastling & 2) > 0)) {
                    transitions.Add(new Transition(
                        TransitionType.Castling,
                        (Castling)BlackCastling,
            ...
            if (this.Position[ixEndSquare] != (sbyte)Piece.Empty) {
                transitions.Insert(0, Transition.Remove(ixEndSquare, Position[ixEndSquare]));
                move.HalfMoveCount = 0;
            }

            move.UciStart = ixStartSquare;
            move.UciEnd = ixEndSquare;
            move.UciPiece = Math.Abs((int)Position[ixStartSquare]);
        ...
        */
        ...
        private List<int> FindOrigins(string strPiece, int start) {
            ...
            Piece startPiece = (Piece)((sbyte)piece * ColorToMove);
                ...
                while ((ixSquare & 0x88) == 0) {
                    ...
                    if (this.Position[ixSquare] != 0) {
                        ...
                        if (ixSquare == ixPiece) {
                            ...
                            var pinPiece = (ColorlessPiece)Math.Abs((sbyte)pinner);
